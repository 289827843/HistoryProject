...
public class UcenterPermissionServiceImpl extends AbstractService<UcenterPermission> implements UcenterPermissionService{
     ...
     * 权限删除
     * @param permission
     * @return 1或者0或者2 1代表权限删除成功 0代表权限删除失败 2代表权限被角色引用 不能删除
     */
    @Override
    public int deletePermission(UcenterPermission permission) {
        int result = ucenterPermissionMapper.selectRolePermissionByPerId(permission.getPermissionId());
        if(result>0){
            return 2;//权限被角色引用 不能删除
        }
