...
public class UcenterRoleServiceImpl extends AbstractService<UcenterRole> implements UcenterRoleService{
     ...
     * 角色删除
     * @param role
     * @return 1或者0或者2 1代表角色删除成功 0代表角色删除失败 2角色被用户引用
     */

    @Override
    public int deleteRole(UcenterRole role) {

        int result = ucenterRoleMapper.selectUserRoleByRoleId(role.getRoleId());
        if (result>0){
            return 2;//角色被用户引用 不能删除
        }
        ...
        //删除角色权限对应表
        ucenterRoleMapper.deleteRoleAndPermission(role);
